...
// ADD - kskim (gradient hook)
void set_hook_outputs(network* net, dict* dict_ptr) {
    dict* new_dict = create_dict();

    for (int i = 0; i < net->n; i++) {
        dict* layer_dict = create_dict();
        add_key_dict(layer_dict, "layer", (int)net->layers[i].type, Int, 0);
        add_key_dict(layer_dict, "data", net->layers[i].output, FloatArray, net->layers[i].outputs * net->layers[i].batch);
        add_key_dict(layer_dict, "w", net->layers[i].out_w, Int, 0);
        add_key_dict(layer_dict, "h", net->layers[i].out_h, Int, 0);
        add_key_dict(layer_dict, "c", net->layers[i].out_c, Int, 0);
        add_key_dict(layer_dict, "n", net->layers[i].n, Int, 0);
        add_key_dict(layer_dict, "bias", net->layers[i].biases, FloatArray, net->layers[i].nbiases);
        add_key_dict(layer_dict, "new_coords", net->layers[i].new_coords, Int, 0);

        char* key = xcalloc(10, sizeof(char));
        sprintf(key, "layer_%d", i);
        add_key_dict(new_dict, key, layer_dict, DictPtr, 0);
    }
    add_key_dict(dict_ptr, "layers_output", new_dict, DictPtr, 0);
}

void backward_network_using_hook(network net, network_state state, dict* dict_ptr)
{
    int i;
    float* original_input = state.input;
    float* original_delta = state.delta;
    ...
    for (i = net.n - 1; i >= 0; --i) {
        ...
        add_key_dict(layer_dict, "layer", (int)l.type, Int, 0);
        add_key_dict(layer_dict, "data", l.delta, FloatArray, l.outputs);
        add_key_dict(layer_dict, "w", l.w, Int, 0);
        add_key_dict(layer_dict, "h", l.h, Int, 0);
        add_key_dict(layer_dict, "c", l.c, Int, 0);
    ...
    add_key_dict(dict_ptr, "layers_gradient", new_dict, DictPtr, 0);
}

int entry_index2(layer l, int batch, int location, int entry) {
    int n = location / (l.w * l.h);
    int loc = location % (l.w * l.h);

    return batch * l.outputs + n * l.w * l.h * (5 + l.classes) + entry * l.w * l.h + loc;
}

void* network_predict_using_gradient_hook(network* net, image im)
{
    dict* dict_ptr = create_dict();
...
#ifdef GPU
    if (gpu_index >= 0) {
        int input_size = get_network_input_size(*net);
        int output_size = get_network_output_size(*net);
        state.index = 0;
        state.net = *net;
        state.input = net->input_state_gpu;
        memcpy(net->input_pinned_cpu, im.data, input_size * sizeof(float));

        float* y_temp = xcalloc(5, sizeof(float));
        //memset(y_temp, 0, sizeof(float) * 5);

        if (net->layers[net->n - 1].truths) output_size = net->layers[net->n - 1].truths * net->batch;
        *net->input_gpu = cuda_make_array(im.data, input_size);
        *net->truth_gpu = cuda_make_array(y_temp, output_size);
    ...
    out = get_network_output(*net);
    // fwd_output
    add_key_dict(dict_ptr, "fwd_output", out, FloatArray, net->outputs);
    set_hook_outputs(net, dict_ptr);
    //state.input = out;
    //state.delta = out;
    net->layers[net->n - 1].delta = net->layers[net->n - 1].output;
    backward_network_using_hook(*net, state, dict_ptr);
    //set_hook_gradients(net, hook);
    ...
    for (int i = 0; i < net->n; i++) {
        if (net->layers[i].type == YOLO) {
            layer l = net->layers[i];
            int n = 1;
            printf("===== [YOLO LAYER-%2i] ======\n", i);
            for (int k = 0; k < 85; k++) {
                //for (int n = 0; n < 1; n++) {
                // GET BBOX
                int idx = entry_index2(l, 0, n * l.w * l.h + 3, k);
                printf("[%2i] %f\n", k, l.output[idx]);
                //}
            }
        }
...
void* get_network_info(network* net) {
    dict* dict_ptr = create_dict();
    add_key_dict(dict_ptr, "n", net->n, Int, 0);
    add_key_dict(dict_ptr, "w", net->w, Int, 0);
    add_key_dict(dict_ptr, "h", net->h, Int, 0);
    add_key_dict(dict_ptr, "c", net->c, Int, 0);
    add_key_dict(dict_ptr, "inputs", net->inputs, Int, 0);
    add_key_dict(dict_ptr, "outputs", net->outputs, Int, 0);
    add_key_dict(dict_ptr, "input", net->input, FloatPtr, 0);
    //add_key_dict(net->class)
    return dict_ptr;
}

void* get_network_layers(network* net) {
    dict* dict_ptr = create_dict();
    for (int i = 0; i < net->n; i++) {
        layer l = net->layers[i];
        dict* layer_dict = create_dict();
        add_key_dict(layer_dict, "idx", i, Int, 0);
        add_key_dict(layer_dict, "type", (int)l.type, Int, 0);
        add_key_dict(layer_dict, "inputs", l.inputs, Int, 0);
        add_key_dict(layer_dict, "w", l.w, Int, 0);
        add_key_dict(layer_dict, "h", l.h, Int, 0);
        add_key_dict(layer_dict, "c", l.c, Int, 0);
        add_key_dict(layer_dict, "n", l.n, Int, 0);
        add_key_dict(layer_dict, "outputs", l.outputs, Int, 0);
        add_key_dict(layer_dict, "out_w", l.out_w, Int, 0);
        add_key_dict(layer_dict, "out_h", l.out_h, Int, 0);
        add_key_dict(layer_dict, "out_c", l.out_c, Int, 0);
        add_key_dict(layer_dict, "output", l.output, FloatPtr, 0);
        add_key_dict(layer_dict, "delta", l.delta, FloatPtr, 0);
        add_key_dict(layer_dict, "biases", l.biases, FloatArray, l.nbiases);
        add_key_dict(layer_dict, "new_coords", l.new_coords, Int, 0);
        add_key_dict(layer_dict, "classes", l.classes, Int, 0);
        int n = l.type == YOLO ? l.n : 0;
        add_key_dict(layer_dict, "mask", l.mask, IntArray, n);

        char* key = xcalloc(10, sizeof(char));
        sprintf(key, "layer_%d", i);
        add_key_dict(dict_ptr, key, layer_dict, DictPtr, 0);
    }
